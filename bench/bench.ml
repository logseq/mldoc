...
let main () =
  Command.run
    (Bench.make_command
       ...
       ; Bench.Test.create ~name:"Mldoc Org mode parser (outline only)" (fun () ->
             ignore (parse outline_config doc_org))
       ; Bench.Test.create ~name:"Mldoc Markdown parser" (fun () ->
             ignore (parse config syntax_md))
       ; Bench.Test.create ~name:"Mldoc Markdown parser (outline only)" (fun () ->
             ignore (parse outline_config syntax_md))
       ])
