  ...
  let remove_empty_branch l =
    ...
    let rec aux z =
      if is_end z then
        ...
        match node z with
        | Branch [] -> remove z >>| (fun z' -> aux (next z')) |? aux (next z)
        | _ -> aux (next z)
    in
    aux z
  in
...
let parse opml_string =
  ...
  let _ = parse_header input in
  body_to_tree input |> tree_to_string_tree_type
  |> Markdown_transformer.String_Tree_Value.to_value |> Tree_type.of_value
  |> Tree_type.to_blocks
